...
def get_polinom_var(k):#Принимает натуральное значение степени полинома. Возвращает список его переменных.
...
def get_polinom_index(k): #Принимает натуральное значение степени полинома. Возвращает список его коэффициентов.
...
def get_equation_terms(pln_ind, pln_var): # Принимает списки коэффициентов и переменных.
...
def create_file(text): # Принимает текст и создаёт файл с этим текстом.
    with open('4_1_file', 'w') as f:
        f.write(text)
    f.close()
